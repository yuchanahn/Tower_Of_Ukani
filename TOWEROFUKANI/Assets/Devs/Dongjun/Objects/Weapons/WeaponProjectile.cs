...
public class WeaponProjectile : PoolingObj
{
    ...
    {
        velocity.y -= projectileData.gravity;
    ...
    {
        if (projectileData.curTravelDist >= projectileData.maxTravelDist)
    ...
    {
        Vector2 pos = (transform.position + (transform.right * detectSize.x * 0.5f)) - (transform.right * (projectileData.moveSpeed * Time.fixedDeltaTime));
        float rot = transform.rotation.eulerAngles.z;

        RaycastHit2D[] hits =
            Physics2D.BoxCastAll(pos, detectSize, rot, transform.right, projectileData.moveSpeed * Time.fixedDeltaTime, detectLayers);
    ...
    public void SetData(WeaponProjectileData projectileData)
    {
        ...
        velocity = projectileData.moveSpeed * transform.right;
    }
    #endregion
}
