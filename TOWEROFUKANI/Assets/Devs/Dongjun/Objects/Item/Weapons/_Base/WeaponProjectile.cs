...
public class WeaponProjectile : PoolingObj
{
    ...
    {
        velocity.y -= projectileData.gravity.curStat;
    ...
    {
        if (projectileData.curTravelDist >= projectileData.maxTravelDist.curStat)
    ...
    {
        Vector2 pos = (transform.position + (transform.right * detectSize.x * 0.5f)) - (transform.right * (projectileData.moveSpeed.curStat * Time.fixedDeltaTime));
        float rot = transform.rotation.eulerAngles.z;

        RaycastHit2D[] hits =
            Physics2D.BoxCastAll(pos, detectSize, rot, transform.right, projectileData.moveSpeed.curStat * Time.fixedDeltaTime, detectLayers);
        ...
        if (hits.Length != 0)
        {
            ...
            for (int i = 0; i < hits.Length; i++)
            {
                ...
                if (mob != null)
                {
                    mob.Hit(projectileData.damage.curStat);
                    OnHit(hitPos);
                    return;
                }
            }
    ...
    public void SetData(WeaponProjectileData projectileData)
    {
        ...
        velocity = projectileData.moveSpeed.curStat * transform.right;
    }
    #endregion
}
